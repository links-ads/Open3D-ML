...
def parse_args():
    ...
    parser.add_argument(
        "-p", "--pipeline", help="pipeline", default="SemanticSegmentation"
    )
    ...
    parser.add_argument("--device", help="devices to run the pipeline", default="cuda")
    parser.add_argument(
    ...
    parser.add_argument(
        "--port", help="port for distributed training, default: 12355", default="12355"
    )
    parser.add_argument(
        "--backend",
        help="backend for distributed training. One of (nccl, gloo)}, default: gloo",
        default="gloo",
    )
...
def main():
    ...
    if framework == "torch":
        ...
        os.environ["TF_CPP_MIN_LOG_LEVEL"] = "1"  # Disable INFO messages from tf
    ...
    if args.cfg_file is not None:
        ...
        if (args.pipeline and args.model and args.dataset) is None:
            raise ValueError(
                "Please specify pipeline, model, and dataset " + "if no cfg_file given"
            )
...
def main_worker(
    ...
    model = Model(**cfg_dict_model, mode=args.mode)
    pipeline = Pipeline(
        model, dataset, distributed=args.distributed, **cfg_dict_pipeline
    )
