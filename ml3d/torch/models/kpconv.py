...
class KPFCNN(BaseModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if reduce_fc:
            ...
            self.head_mlp = UnaryBlock(
                out_dim, cfg.first_features_dim, False, 0, l_relu=cfg.get("l_relu", 0.1)
            )
            self.head_softmax = UnaryBlock(
                cfg.first_features_dim, self.C, False, 0, l_relu=cfg.get("l_relu", 0.1)
            )
    ...
    def transform(self, data, attr, is_test=False):
        ...
        while curr_num_points < min_in_points:
            ...
            mask_inds, p0 = self.trans_point_sampler(
                pc=new_points,
                feat=feat,
                label=sem_labels,
            ...
            if n > residual_num_points:
                input_inds = np.random.choice(
                    n, size=residual_num_points, replace=False
                )
    ...
    def update_probs(self, inputs, results, test_probs):
        ...
        for b_i, length in enumerate(lengths):
            ...
            test_probs[proj_mask] = (
                self.test_smooth * test_probs[proj_mask]
    ...
    def inference_end(self, inputs, results):
        ...
        for b_i, length in enumerate(lengths):
            ...
            proj_inds = r_inds_list[b_i]
            proj_mask = r_mask_list[b_i]
            self.test_probs[proj_mask] = (
                self.test_smooth * self.test_probs[proj_mask]
                + (1 - self.test_smooth) * probs
            )
            i0 += length
    ...
    def augmentation_transform(
        self, points, normals=None, verbose=False, is_test=False
    ):
        ...
        noise = (
            np.random.randn(points.shape[0], points.shape[1]) * self.cfg.augment_noise
        ).astype(np.float32)
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        if in_dim != out_dim // 4:
            self.unary1 = UnaryBlock(
                in_dim, out_dim // 4, self.use_bn, self.bn_momentum, l_relu=l_relu
            )
        else:
...
def load_kernels(radius, num_kpoints, dimension, fixed, lloyd=False):
    ...
    # Kernel_file
    kernel_file = join(
        kernel_dir, "k_{:03d}_{:s}_{:d}D.npy".format(num_kpoints, fixed, dimension)
    )
...
def batch_neighbors(queries, supports, q_batches, s_batches, radius):
    ...
    dense_idx = ragged_to_dense(
        idx, splits, max_nbrs, torch.Tensor([supports.shape[0]]).to(torch.int32)
    ).squeeze(2)
...
def batch_grid_subsampling(
    ...
    if (features is None) and (labels is None):
        s_points, s_len = subsample_batch(
