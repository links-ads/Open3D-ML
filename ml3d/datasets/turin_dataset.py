...
class TurinDataset3DSplit():
    def __init__(self, dataset, split='training', scale=np.array([1.0, 1.0, 1.0]), offset=np.array([0.0, 0.0, 0.0])):
    ...
    def get_data(self, idx):
        ...
        points = np.concatenate([np.expand_dims(data.x,axis=1),
                                 np.expand_dims(data.y,axis=1),
                                 np.expand_dims(data.z,axis=1)],axis=1)
        points = points * self.scale - self.offset
        feat = np.concatenate([np.expand_dims(data.red,axis=1),
                                 np.expand_dims(data.green,axis=1),
                                 np.expand_dims(data.blue,axis=1)],axis=1).astype(np.uint8).astype(np.float32)
        if (self.split != 'test'):
...
class TurinDataset3D(Custom3D):
    def __init__(self,
        ...
        self.train_files = [f for f in glob.glob(self.train_dir + "/*."+self.file_format)]
        self.val_files = [f for f in glob.glob(self.val_dir + "/*."+self.file_format)]
        self.test_files = [f for f in glob.glob(self.test_dir + "/*."+self.file_format)]
    ...
    def get_split(self, split):
        ...
        """
        return TurinDataset3DSplit(self, split=split, scale=self.scale, offset=self.offset)
    ...
    def save_test_result(self, results, attr):
        ...
        assert len(las.x) == len(results['predict_labels']), "Prediction and points are not of the same size"
